...
if (isset($_POST['simpan_transaksi'])) {
    ...
    $query_baju = mysqli_query($koneksi, "SELECT harga, stok FROM baju WHERE id = $id_baju");
    $data_baju  = mysqli_fetch_assoc($query_baju);

    if ($data_baju['stok'] < $jumlah) {
        echo "<script>alert('Stok baju tidak mencukupi!');</script>";
    } else {
        $total_harga = $data_baju['harga'] * $jumlah;
        $kembalian   = $bayar - $total_harga;
        $tgl         = date('Y-m-d H:i:s');

        mysqli_query($koneksi, "INSERT INTO penjualan VALUES(NULL, '$no_faktur', '$tgl', '$id_pelanggan', '$id_baju', '$jumlah', '$total_harga', '$bayar', '$kembalian', '$status', '$id_user')");

        mysqli_query($koneksi, "UPDATE baju SET stok = stok - $jumlah WHERE id = $id_baju");
...
$baju      = mysqli_query($koneksi, "SELECT * FROM baju WHERE stok > 0");
$pelanggan = mysqli_query($koneksi, "SELECT * FROM pelanggan");
$transaksi = mysqli_query($koneksi, "SELECT p.*, c.nama as nama_pelanggan, b.nama_baju, b.ukuran FROM penjualan p JOIN pelanggan c ON p.id_pelanggan = c.id JOIN baju b ON p.id_baju = b.id ORDER BY p.id DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <title>Transaksi Penjualan Baju</title>
...
<body class="bg-slate-50 min-h-screen">
    <div class="flex">
        <div class="w-64 bg-slate-800 min-h-screen text-white p-4 space-y-2">
            ...
            <a href="baju.php" class="block py-2.5 px-4 text-slate-300 hover:bg-slate-700 rounded">Data Baju</a>
        ...
        <div class="flex-1 p-8">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold">Transaksi Penjualan Baju</h2>
                <a href="cetak_laporan.php" target="_blank" class="bg-emerald-600 text-white px-4 py-2 rounded hover:bg-emerald-700 font-semibold">Cetak Laporan</a>
            </div>
            ...
            <form method="POST" class="bg-white p-6 rounded-lg shadow mb-8 grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block mb-1 text-sm">Pilih Baju</label>
                    <select name="id_baju" required class="w-full border p-2 rounded">
                        <?php while ($b = mysqli_fetch_assoc($baju)): ?>
                            <option value="<?= $b['id'] ?>"><?= $b['nama_baju'] ?> [Size: <?= $b['ukuran'] ?>] - Rp <?= number_format($b['harga']) ?> (Stok: <?= $b['stok'] ?>)</option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div>
            ...
            <table class="w-full bg-white rounded-lg shadow overflow-hidden">
                ...
                <tbody>
                    ...
                    <tr class="border-t">
                        ...
                        <td class="p-3"><?= $row['nama_baju'] ?> (<?= $row['ukuran'] ?>)</td>
